...
export class EncryptionService {
  ...
  constructor() {
    ...
    if (raw) {
      ...
      if (process.env.NODE_ENV === 'production') {
        throw new Error('CRYPTO_KEY is required in production for token encryption');
      }
      this.logger.warn('CRYPTO_KEY not set — stored tokens will not be encrypted');
      this.key = null;
    }
  }
  ...
  decrypt(value: string): string {
    ...
    if (iv.length !== IV_LENGTH || authTag.length !== AUTH_TAG_LENGTH) return value;
