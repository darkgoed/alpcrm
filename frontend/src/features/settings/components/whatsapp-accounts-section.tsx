...
function AccountCard({ account, onChanged }: { account: WhatsappAccount; onChanged: () => void }) {
  ...
  async function handleTest() {
    if (!form.metaAccountId || !form.token) {
      setTestResult({ ok: false, label: 'Informe o Phone Number ID e um novo Access Token para testar' });
      return;
    }
    setTesting(true);
    setTestResult(null);
  ...
  async function handleSave() {
    const required: Array<keyof Pick<AccountForm, 'name' | 'phoneNumber' | 'metaAccountId' | 'wabaId' | 'verifyToken'>> = [
      'name',
      'phoneNumber',
      'metaAccountId',
      'wabaId',
    ...
    const payload: Parameters<typeof updateWhatsappAccount>[1] = {
      name: form.name,
          ...
          {editing ? (
            <div className="px-4 py-4 space-y-4">
              ...
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                ...
                <div className="space-y-1.5">
                  <label className="text-xs font-medium text-muted-foreground">Phone Number ID</label>
                  <Input value={form.metaAccountId} onChange={(e) => set('metaAccountId', e.target.value)} />
                </div>
                <div className="space-y-1.5">
                  <label className="text-xs font-medium text-muted-foreground">WABA ID</label>
                ...
                <div className="space-y-1.5 sm:col-span-2">
                  ...
                  <Input
                    type="password"
                    placeholder="Preencha apenas para substituir o token atual"
                    value={form.token}
                    onChange={(e) => set('token', e.target.value)}
                  />
                </div>
                <div className="space-y-1.5 sm:col-span-2">
                  <label className="text-xs font-medium text-muted-foreground">Novo App Secret</label>
                  <Input
                    type="password"
                    placeholder="Preencha apenas para substituir o App Secret atual"
              ...
              <div className="rounded-lg border border-border/70 bg-muted/30 p-4 space-y-3">
                <div className="flex items-center gap-2">
                  <span className="text-xs text-muted-foreground w-28 shrink-0">Webhook URL</span>
                  <code className="flex-1 rounded bg-background px-2 py-1 text-xs font-mono truncate">{webhookUrl}</code>
                  <CopyButton value={webhookUrl} label="URL do webhook" />
                </div>
                <div className="flex items-center gap-2">
                  <span className="text-xs text-muted-foreground w-28 shrink-0">Verify Token</span>
                  <code className="flex-1 rounded bg-background px-2 py-1 text-xs font-mono truncate">{form.verifyToken}</code>
                  <CopyButton value={form.verifyToken} label="verify token" />
                  <Button
                    variant="ghost"
                    size="icon"
